...
13. ⬜ **ActivityFeed.svelte** (875, biggest). Group filter chips (All/Sync/Pull/Delete/Drift/Conflicts/Bridge/Errors/System) deserve tone-coding — currently text-only. Burst-mode banner, sticky-scroll "N new" pip, row group-collapse, kind→icon mapping all need a canon pass. This is the page that benefits MOST from the rhythm. Save for when you've got runway.
