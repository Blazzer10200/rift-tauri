...
## Session 2026-05-31 (a) — #265 live-SFTP transfer coverage (tests only, no ship)
Closed the long-open #265 "real-SFTP vs mocks" gap on the transfer layer. **Additive only** — 7 `#[ignore]` integration tests in `src-tauri/src/sftp/integration_tests.rs` (new module, wired `#[cfg(test)] mod integration_tests;` in `sftp/mod.rs`). No production code touched → cannot break the build/runtime.
