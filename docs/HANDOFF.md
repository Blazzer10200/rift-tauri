...
- **Redesign is NOT as complete as earlier claimed** — a 4-agent verified audit (`docs/design/redesign-gap-audit.md`, 2026-06-02) found real gaps the narrative missed: **Settings General section + RCON console + danger zone MISSING**; **Chat History is list-only** (detail pane + ChatRail unbuilt); **Files detail pane / Onboarding restyle unstarted**; rail-expand + ⌘K-top-bar-search + PageHeader/PageToolbar not done. **`docs/design/redesign-gap-audit.md` is the corrected baseline + sequenced multi-session plan — read it before resuming redesign work.** Done: tokens, Home, Sync (tabbed hub), Chat core (flat timeline), Settings Appearance/A11y/Assistant/About.
