...
- **NEXT:** ship (g) as a release (bump 3 files + Cargo.lock + CHANGELOG); optional: picker↔pinned-model display sync (picker can now visually diverge from a chat's pinned model after a mid-chat switch); recovery for already-bricked chats (their on-disk transcript damage predates the fix — new chat needed).
...
## Session 2026-05-31 (e) — side panel merged into one live surface (shipped in v0.4.44 — detail in git log)
Right panel reworked from a 2-tab Session/Activity split into ONE scrolling surface (`ActivityPanel.svelte` rewrite, `SidePanel.svelte` thin wrapper): Now strip → Running → Tasks → Outputs → Sources → Tool mix → Insights. `liveActivity` (helpers.ts) surfaces ALL pending tools + thinking state, labelled via `captionForTool`. Dropped the redundant "This session" stat card (status bar already has tok/s·tools·cost). Session (g) above adds resize + collapsible review zone on top of this.
