...
## Session 2026-06-11 (cont.113) — Activity panel polish

One commit `b6dfabe`, verified (svelte-check 0/0 · vitest 122/122 · live CDP 0 console errors). All in `ActivityPanel.svelte`:
...
**Finding (not fixed):** ctx meter blank on restored convos — `lastTurnUsage` is live-only, never persisted; fix = persist usage in conversation store. ISSUES candidate.
...
- **ISSUES #31** (legacy `base_url`/`provider_model` cmds · turn.rs 401-dup helper · blocking-fs-in-async · **Fable dead-branch sweep after Jun 22**) + **#30** (reload-while-on-chat init race, semi-repro) + ctx-meter-on-restore (above).
