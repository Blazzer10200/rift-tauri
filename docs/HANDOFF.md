...
### RESUME HERE — redesign, remaining screens (biggest first)
Dev server + CDP left running (`rift-tauri.exe` dev binary, localhost only). Use COMPONENT_MAP.md as the map; lift exact CSS from `mockup/styles/*.css` (home was in `mix.css`). All remaining are bigger per-screen jobs — do fresh, one at a time, CDP-verify each:
1. **Settings full single-scroll rebuild** — current is legacy slide-over-style `Settings.svelte` (1652L, section-switch). Redesign wants ONE scroll doc + sticky scroll-spy index, 5 sections (General/Appearance/Server/Assistant/About). **MUST preserve `accessibility.svelte.ts` a11y options** (not in prototype). Adopt the new `Select`. Appearance controls already built — fold in.
2. **Sync tabbed hub** (`sync.css`/`sync.jsx`) — persistent status header above Drift/Conflicts/**Activity** tabs. **This is where Activity gets demoted** from top-level: only after the Activity tab exists, remove `activity` from `WORKSPACES`/`WORKSPACE_IDS` + remap. (Deferred tonight to avoid stripping the feature.)
3. **Files detail pane** (`files.css`) · **Chat flat-timeline** + tool renderers (`chat-thread.css`, biggest) · **Onboarding** restyle (`onboarding.css`).
4. Shell polish: rail active = accent thin-left-bar + "calm not blocky" (tokens already did 80%). Default workspace still `chat` (could switch to `home`).
