...
- **✅ RESOLVED in-tree — unshipped, delete on `/git-ship` (code re-verified 2026-06-05):** #31 (fast-mode hidden until wired, cont.57), #32 (`\\?\` strip, cont.54), #33 (Harness avg-dead-wait recompute on load, cont.54), #34 (palette "Go to Home", cont.54), #35 (per-recent "Forget" button, cont.54), #36 (scroll-spy bottom-detect for last section, cont.58). Each carries a `✅` tag + verified file:line on its block below. Plus a **Harness no-scroll redesign** (KPI rail + collapsible "Show details"; not an ISSUES item — detail in HANDOFF cont.54). Blocks kept until ship so `git log` preserves them.
...
- ~~#36 Settings scroll-spy last-section ("About") highlight~~ (✅ resolved in-tree cont.58 — bottom-detect) · #29 CSP `style-src 'unsafe-inline'` (Tailwind-blocked) · ~~#31 fast-mode toggle cosmetic~~ (✅ resolved in-tree cont.57 — hidden until wired) · Wave-1 LOWs #91-#134 — clippy/doc/perf nits (see `docs/archive/audit-history.md`).
