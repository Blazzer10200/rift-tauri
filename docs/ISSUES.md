...
- **Where:** As of 2026-05-29: 113 `#[test]` fns + 39 vitest tests. `drift_scanner` now has offline coverage via the `SftpOps` trait (#265 Wave B Phase 1, `64b79ef`). See #265 for the strategy + remaining uncovered-HIGH-risk list.
...
- **Status (2026-05-29):** 113 `#[test]` fns + 39 vitest tests. Wave A landed: `path_guard::validate_local_listable` (7 tests), `SyncSnapshot::count_under` / `replace_under` / mtime tolerance (3 tests). **Wave B Phase 1 landed (`64b79ef`):** `SftpOps` trait (`sftp/sftp_ops.rs`) abstracts the 8 sync-facing `SftpClient` methods; `DriftScanner` now takes `&dyn SftpOps`; `MockSftp` + 2 first-scan drift tests (ToPull / ToPush) run offline. `SyncSnapshot::for_path` was already present (Wave A enabler). Flush counter math deferred (see #21.1). Remaining structural blockers: `AutoSyncEngine` needs `AppHandle`; `BRIDGE: OnceLock` set-once. Uncovered HIGH-risk modules: `sftp/transfer.rs`, `auto_sync/flush.rs`, `assistant/remote_bridge.rs`, `assistant/mod.rs`, `assistant.svelte.ts`, `drift_watcher.rs` (`drift_scanner.rs` now has offline coverage).
