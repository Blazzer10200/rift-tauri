...
**Origin:** during the read-only audit swarm (`scripts/audit-swarm.workflow.js`, ~550 agents, find→verify→synthesize), the question came up: if a parallel swarm can *review* the whole codebase accurately, can a swarm safely *apply the fixes* too — not just describe them?
