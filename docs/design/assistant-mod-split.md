...
> Brief for the #20 backend split. Authoritative state in [src-tauri/src/assistant/mod.rs](../../src-tauri/src/assistant/mod.rs) — was **4331 lines**; **2917L after R1/R3/R4/R5/R7 shipped 2026-06-09** (cont.97: `9338179`, `3602312`, `a8a2c0b`, `e091890`, `99864c4`). Follows the Rust precedent (`sync/auto_sync/flush.rs`+`watch.rs`, and `assistant/`'s own existing children `ask_user / git_local / mcp_server / permission / session_log`): new **sibling child modules** under `src-tauri/src/assistant/`, NOT the TS free-fn pattern. **REMAINING: R2 (config) · R6 (oneshot) · R8 (turn — last).** Pre-split line numbers below are stale for shipped modules — re-grep before extracting. Two execution lessons: (1) `#[tauri::command]` re-exports MUST be glob (`pub use module::*;`) — named re-exports strand the macro-generated `__cmd__*` items and lib.rs registration breaks; (2) `pub(super)` on a child item is visible to the whole `assistant` subtree, so siblings (e.g. auth_update → cli_install) import via `super::cli_install::{…}` w/o widening visibility.
