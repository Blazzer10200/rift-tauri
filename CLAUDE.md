...
| File | Lines | Notes |
|---|---|---|
| `src-tauri/src/assistant/turn.rs` | 1393 | live-turn nervous system — session registry + steer + permission plumbing + `assistant_send` /stop/steer + per-turn env-snapshot reminder (v0.8.21). Split COMPLETE cont.98: mod.rs is a ~310L hub; siblings `config` 252 · `oneshot` 548 (enhance/title only — summarize/remint removed in the strip) · `bridge` 284 (loopback UI bridge, v0.8.21) · R1-R7 — pattern ref: `docs/design/assistant-mod-split.md` |
| `src-tauri/src/assistant/mcp_server.rs` | 1075 | stdio JSON-RPC MCP server — `read_file/list_dir/grep` + `git_*` + bridge-gated `ask_user/open_browser/notify` (v0.8.21; sync/remote_bash stay dead) |
...
| `src-tauri/src/usage/limits.rs` | 176 | the ONLY usage module left — v0.8.20 OAuth `/usage` rate-limit fetch (CLI token READ-ONLY, never refresh it) |
| `src-tauri/src/lib.rs` | ~230 | tauri command registry (per-domain handlers in `commands/*.rs`) |

Backend dirs (current): `assistant/ browser/ commands/ diagnostics/ state/ stt/ usage/` + `lib.rs main.rs secrets.rs update_service.rs`. **Minimal-core strip 2026-06-12 (buddy release):** `swarm/` deleted, `usage/` slimmed to `limits.rs` only (SQLite cockpit + rusqlite dep gone), `assistant/session_log.rs` gone, compaction (oneshot summarize/remint + config fields) gone, custom providers + compression proxy gone (first-party Anthropic only). The entire `sftp/ sync/ bootstrap/ edit/ tunnel/ transport/ bridge/ rcon/ profile/` set is gone (pure-assistant rip), plus `local_fs.rs path_guard.rs` and the `remote_state`/`sync_snapshot` state caches.

Frontend hot files (re-measured 2026-06-12, post-strip): `Composer.svelte` 2036L (split COMPLETE C1-C7 — eight children under `components/assistant/composer/` [QueueRail 322 · SettingsMenu 370 · EnhanceBar 264 · LivePills 212 · PermMenu 147 · AttachmentsRow 114 · MentionPopover 110 · SlashMenu ~165] + `modelMatrix.ts` shared model/effort/perm tables; brief `docs/design/composer-split.md` kept as the component-split pattern ref), `shell/ChatTabsBar.svelte` 1683L (H0 done — brief: `chattabsbar-split.md`), `ToolChip.svelte` 1554L, `assistant.svelte.ts` 1509L (split COMPLETE cont.97 — M0-M9 carved into `src/lib/state/assistant/`, incl. `streaming.ts` stream pump + `send.ts` orchestrator; regression net = `assistant.playback.test.ts`; `compaction.ts` + `sessionLog.ts` deleted in the strip), `MessageBubble.svelte` 1471L (H0 done — brief: `messagebubble-split.md`; still renders legacy boundary pills from old convos), `HistoryDrawer.svelte` 1279L, `Markdown.svelte` 1120L, `SettingsPage.svelte` 1102L (cont.88 layout; provider/compression/compaction sections removed in the strip), `AssistantPane.svelte` 981L. **Workspaces are 3 now: Home · Chat · Settings** — `HarnessPage`/`CostPage`/`SwarmPage` + `workspaces/helpers.ts` deleted; `usage.svelte.ts` is gauges-only (`rateLimits` + `refreshRateLimits`). Pure-helper modules w/ vitest nets (don't re-inline): `composer/helpers.ts` · `composer/modelMatrix.ts` · `bubble/helpers.ts` · `tabsbar/helpers.ts` · `$lib/actions/portal.ts` (canonical `portal` + `portalFocus`). **Effort mapping lockstep:** `state/assistant/helpers.ts::effortToFlag` ↔ `turn.rs` match ↔ `modelMatrix.ts` tables (none→low · quick→medium · smart→high default · deep→xhigh · ultra→xhigh+ultracode; Sonnet caps at smart, Haiku excluded) — change all three together. Deleted in the conversion: `SyncPage`, `ActivityFeed`, the `diagnostics`/`connection`/`sync-*` stores, all server/onboarding dialogs.
