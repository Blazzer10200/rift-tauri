...
| File | Lines | Notes |
|---|---|---|
| `src-tauri/src/assistant/turn.rs` | 1442 | live-turn nervous system — session registry + steer + permission plumbing + `assistant_send` /stop/steer + per-turn env-snapshot reminder (v0.8.21). Split COMPLETE cont.98: mod.rs is a ~310L hub; siblings `config` 569 · `oneshot` 734 · `bridge` 284 (loopback UI bridge, v0.8.21) · R1-R7 — pattern ref: `docs/design/assistant-mod-split.md` |
| `src-tauri/src/assistant/mcp_server.rs` | 1073 | stdio JSON-RPC MCP server — `read_file/list_dir/grep` + `git_*` + bridge-gated `ask_user/open_browser/notify` (v0.8.21; sync/remote_bash stay dead) |
