...
/** Canonicalize a root path for comparison: normalize `\` → `/` (saved convo
 *  roots and backend-canonical paths can disagree on separator for the same
 *  folder), strip a trailing separator, and lowercase (Windows paths are
 *  case-insensitive + drive-letter casing varies). The single home for the
