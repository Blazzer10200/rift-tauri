...
{#if turnStats.files > 0}
  <div class="turn-summary" data-auto={autoApplied ? "true" : null} class:mode-bypass={bypassApplied} in:fade={{ duration: reducedMotion ? 0 : 160 }}>
    <div class="ts-stats">
      ...
      {#if message.fast}<span class="ts-dot" aria-hidden="true"></span><span class="ts-cost mono" use:tooltip={"This provider confirmed that the turn ran with higher-speed processing. Fast can use extra credits or premium API pricing."}><Zap size={11} />fast</span>{/if}
    </div>
