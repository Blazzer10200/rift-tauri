<script lang="ts">
  ...
  let { path, onPathChange, onOpenInNewTab, onDropPaths, onDropPathsToFolder, onUploadPaths, onSelectionChange, onActiveEntry, fileFilter = "all", refreshKey = 0 }: Props = $props();
  ...
  function fileStatus(e: LocalEntry): "synced" | "modified" | "conflict" {
    if (e.is_dir) return "synced";
    if (rowStatus(e) === "conflict") return "conflict";
    const rp = connection.remoteForLocalPath(e.path);
    if (rp && connection.dirtyEdits.has(rp)) return "modified";
    return "synced";
  }
  ...
  $effect(() => {
    ...
    const rp = connection.remoteForLocalPath(e.path);
    const lk = !e.is_dir && rp ? connection.lockForRemotePath(rp) : null;
    onActiveEntry?.({
      side: "local",
      name: e.name,
      path: e.path,
      is_dir: e.is_dir,
      size: e.size,
      modifiedLabel: fmtTime(e.mtime),
      modifiedAbs: fmtTimeAbs(e.mtime),
      status: fileStatus(e),
      locked: !!lk,
